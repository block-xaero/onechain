use crate::core::data_structures::skip_list::SkipList;
use libc::mlock;
use libc::munlock;
...
pub fn pin_memory(ptr: *const u8, size: usize) -> Result<(), String> {
    ...
    if result == 0 {
        ...
        Err(format!("Failed to pin memory: {}", std::io::Error::last_os_error()))
    }
}
...
pub fn unpin_memory(ptr: *const u8, size: usize) -> Result<(), String> {
    ...
    if result == 0 {
        ...
        Err(format!("Failed to unpin memory: {}", std::io::Error::last_os_error()))
    }
}